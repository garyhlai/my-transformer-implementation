    ...
    def split(self, split_ratio=0.7, stratified=False, strata_field='label',
        ...
        if not stratified:
            ...
            if strata_field not in self.fields:
                raise ValueError(f"Invalid field name for strata_field {strata_field}")
    ...
    def __init__(self, path, format, fields, skip_header=False,
        ...
        with io.open(os.path.expanduser(path), encoding="utf8") as f:
            ...
            if format in ['csv', 'tsv'] and isinstance(fields, dict):
                if skip_header:
                    raise ValueError(
                        f'When using a dict to specify fields with a {format} file,skip_header must be False andthe file must have a header.'
                    )
...
def check_split_ratio(split_ratio):
    ...
    if isinstance(split_ratio, float):
        ...
        assert 0. < split_ratio < 1., f"Split ratio {split_ratio} not between 0 and 1"
        ...
        raise ValueError(
            f'Split ratio must be float or a list, got {type(split_ratio)}'
        )
...
def rationed_split(examples, train_ratio, test_ratio, val_ratio, rnd):
    ...
    test_len = int(round(test_ratio * N)) if val_ratio else N - train_len
    indices = (randperm[:train_len],  # Train
               randperm[train_len:train_len + test_len],  # Test
               randperm[train_len + test_len:])  # Validation

    return tuple([examples[i] for i in index] for index in indices)
